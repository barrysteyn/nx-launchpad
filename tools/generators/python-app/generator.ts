...
function buildInfraTargets() {
  return {
    ...
    'tf-apply': {
      ...
      dependsOn: ['build'],
      configurations: {
        staging: { command: 'TF_VAR_project_name=$PROJECT_NAME TF_VAR_environment=$ENVIRONMENT terraform apply -auto-approve', cwd: '{projectRoot}/infra/environments/staging' },
        production: { command: 'TF_VAR_project_name=$PROJECT_NAME TF_VAR_environment=$ENVIRONMENT terraform apply -auto-approve', cwd: '{projectRoot}/infra/environments/production' },
      },
    },
    deploy: {
      executor: 'nx:run-commands',
