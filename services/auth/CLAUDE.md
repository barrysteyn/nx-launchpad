...
- **Email handlers use `ctx.waitUntil(sendEmail(...))`** — in Cloudflare Workers, any promise not tied to `ctx.waitUntil()` is cancelled the moment the response is sent. A fire-and-forget `void sendEmail(...)` would silently drop the SES request: no email arrives, no error surfaces, nothing in `wrangler tail` (cancellation isn't a rejection). All three email handlers (`sendResetPassword`, `sendVerificationEmail`, `sendMagicLink`) take `executionCtx` from Hono via `createAuth`'s third parameter and wrap the SES call in `waitUntil` to keep the fetch alive past the response.
